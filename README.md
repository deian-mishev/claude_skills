...
A collection of custom skills for [CLAUDE_CODE] — Anthropic's CLI for Claude.
...
|-------|---------|-------------|
| [music-sort] | `/music-sort` | Organizes a local music folder: reads tags via `ffprobe`, parses filenames for missing metadata, sorts files into `Artist/` subfolders, strips tags via `ffmpeg`, detects duplicates, and generates an `undo.sh` rollback script. |
